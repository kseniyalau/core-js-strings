...
function removeFirstOccurrences(str, value) {
  ...
  const secondPart = str.substring(index + value.length, str.length);
...
function orderAlphabetically(str) {
  ...
  const result = arr.sort((a, b) => a.charCodeAt(0) - b.charCodeAt(0));
...
function isPalindrome(str) {
  let strToCheck = str.replaceAll(' ', '');
  strToCheck = strToCheck.replaceAll(',', '');
  strToCheck = strToCheck.replaceAll('!', '');
  strToCheck = strToCheck.replaceAll('?', '').toLowerCase();

  let half = strToCheck.length / 2;

  if (strToCheck.length % 2 !== 0) {
    half = (strToCheck.length - 1) / 2;
  }

  for (let i = 0; i < half; i += 1) {
    if (strToCheck[i] !== strToCheck[strToCheck.length - 1 - i]) {
      return false;
    }
  }
...
function findLongestWord(sentence) {
  let longestIndex = -1;
  let longestCount = 0;
  const arr = Array.from(sentence.split(' '));

  for (let i = 0; i < arr.length; i += 1) {
    if (longestCount < arr[i].length) {
      longestCount = arr[i].length;
      longestIndex = i;
    }
  }

  return longestIndex >= 0 ? arr[longestIndex] : '';
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const origIndex = orig.indexOf(str[i]);
    if (origIndex >= 0) {
      result += cipher[origIndex];
    } else {
      result += str[i];
    }
  }
